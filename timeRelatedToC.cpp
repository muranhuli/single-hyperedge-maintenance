...
void find(int c, vector<vector<int>> &hyperEdge, vector<int> &edge)
{
    int count = 0;
    for (int i = 0; i < int(hyperEdge.size()); i++)
    {
        if (int(hyperEdge[i].size()) == c)
        {
            edge.push_back(i);
            ++count;
            if (count == 15)
...
{
    ofstream fout1("./result/timeRelatedToC/" + str + "-insert");
    ofstream fout2("./result/timeRelatedToC/" + str + "-erase");
...
int main()
{
    //S_CoGe  S_CoHi  S_DAWN  S_DrAb  S_MiAc  S_NDC  S_TrCl  S_WaTr
    string file = "MaAn";
    // string in = "1 1 1 1 1 1 1";
    // string en = "25 25 16 22 25 25 85 25";
    string in ="1";
    string en ="1784";
